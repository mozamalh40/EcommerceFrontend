...
export const getCategory = (category) => async(dispatch) => {
    try {
        ...
        const { data } = await axios.get(`/api/v1/categorys`,category,config)
        dispatch({
...
export const getAdminCategory = () => async(dispatch) => {
    try {
        ...
        const config = { headers: { "Content-Type": "application/json" } }
        const { data } = await axios.get("/api/v1/admin/categorys",config)
        dispatch({
...
export const getCategoryDetails = (id) => async(dispatch) => {
    try {
        ...
        const config = { headers: { "Content-Type": "application/json" } }
        const { data } = await axios.get(`/api/v1/category/${id}`,config)
        dispatch({
